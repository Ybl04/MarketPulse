...
def get_jobs(keyword: str, country: str = "fr", db: Session = Depends(get_db)):
    """
    Récupère les offres depuis Adzuna, les stocke en base, et les retourne.
    Si une offre existe déjà (via external_id), on ne la duplique pas.
    """
    raw_jobs = fetch_jobs(keyword, country)
    ...
    for raw in raw_jobs:
        ...
        # Déduplication : on vérifie si l'offre existe déjà
        existing = db.query(Job).filter(
            Job.external_id == normalized["external_id"]
        ).first()
...
def get_stats(keyword: str, db: Session = Depends(get_db)):
    """
    Retourne des statistiques agrégées sur les offres stockées en base
